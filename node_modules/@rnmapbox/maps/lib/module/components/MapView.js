 ...
 */
class MapView extends NativeBridgeComponent(React.PureComponent, NATIVE_MODULE_NAME) {
  static defaultProps = {
    projection: 'mercator',
    scrollEnabled: true,
    pitchEnabled: true,
  ...
  async queryRenderedFeaturesAtPoint(coordinate) {
    ...
    const res = await this._runNative('queryRenderedFeaturesAtPoint', [coordinate, getFilter(filter), layerIDs]);
    if (isAndroid()) {
  ...
  async queryRenderedFeaturesInRect(bbox) {
    ...
    if (bbox != null && (bbox.length === 4 || MGLModule.MapboxV10 && bbox.length === 0)) {
      const res = await this._runNative('queryRenderedFeaturesInRect', [bbox, getFilter(filter), layerIDs]);
      if (isAndroid()) {
        return JSON.parse(res.data);
      }
      return res.data;
  ...
  setSourceVisibility(visible, sourceId) {
    let sourceLayerId = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
...
const RCTMGLMapView = requireNativeComponent(NATIVE_MODULE_NAME);
